...
async def create_user_endpoint(user: UserCreate, db: AsyncSession = Depends(get_db), _: UserModel = Depends(get_current_admin)):
    return await create_user_service(db, user)

@app.get("/users/{user_id}", response_model=User)
async def read_user(user_id: int, db: AsyncSession = Depends(get_db), _: UserModel = Depends(get_current_admin)):
    return await get_user_service(db, user_id)

@app.put("/users/{user_id}", response_model=User)
async def update_user_endpoint(user_id: int, user: UserUpdate, db: AsyncSession = Depends(get_db), _: UserModel = Depends(get_current_admin)):
    return await update_user_service(db, user_id, user)

@app.delete("/users/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_user_endpoint(user_id: int, db: AsyncSession = Depends(get_db), _: UserModel = Depends(get_current_admin)):
...
async def get_my_notes(
    ...
    notes_dict = [{"id": n.id, "title": n.title, "content": n.content, "user_id": n.user_id} for n in notes]
